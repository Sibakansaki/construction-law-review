...
export async function generateMnemonic(card) {
  const prompt = `你是一位記憶技巧專家。以下是一個施工法的知識點：
...
/**
 * ② 單一知識點出申論題
 *    新增 starred 參數：若為 true，在 prompt 中強調此為重要考點
 *
 * @param {{ name: string, content: string, examYears?: string[], isExamFocus?: boolean }} card
 * @param {{ starred?: boolean }} options
 */
export async function generateCardQuestion(card, options = {}) {
  ...
  const prompt = `你是一位施工法的考試出題老師。以下是一個知識點：
...
/**
 * ③ 本章出申論題
 *    新增 starredIds 參數：有星星的卡片 id 集合，在 prompt 中標記為重點
 *
 * @param {{ chapter: string, title: string, cards: Array }} chapterData
 * @param {{ starredIds?: Set<string> }} options
 */
export async function generateChapterQuestion(chapterData, options = {}) {
  ...
  const prompt = `你是一位施工法的考試出題老師。以下是本章的知識點清單：
...
export async function gradeAnswer({ question, content, studentAnswer }) {
  const prompt = `你是一位施工法的閱卷老師，請批改以下申論題的答案。
